...
add_filter( 'the_content', 'unique_link_content' );

/* Wraps <blockquote> around quote posts. */
add_filter( 'the_content', 'unique_quote_content' );

/* Filter the content of chat posts. */
add_filter( 'the_content', 'unique_format_chat_content' );
 ...
 */
function unique_link_content( $content ) {
 ...
 * @return string $content
 */
function unique_quote_content( $content ) {
 ...
 */
function unique_format_chat_content( $content ) {
	...
	foreach ( $chat_rows as $chat_row ) {
		...
		if ( strpos( $chat_row, $separator ) ) {
			...
			$speaker_id = unique_format_chat_row_id( $chat_author );
 ...
 */
function unique_format_chat_row_id( $chat_author ) {
	global $_post_format_chat_ids;
 ...
 * @return string
 */
function unique_get_video_embed() {
	global $wp_embed;
	...
	add_filter( 'unique_video_shortcode_embed', array( $wp_embed, 'run_shortcode' ) );
	add_filter( 'unique_video_auto_embed', array( $wp_embed, 'autoembed' ) );
	...
	if ( is_array( $matches ) ) {
		foreach ( $matches  as $value ) {
			...
			$embed = apply_filters( 'unique_video_shortcode_embed', '[embed]' . $value[1]. '[/embed]' );
	...
	if ( empty( $embed ) && get_option( 'embed_autourls' ) ) {
		...
		if ( is_array( $matches ) ) {
			foreach ( $matches  as $value ) {
				...
				$embed = apply_filters( 'unique_video_auto_embed', $value[0] );
 ...
 * @return int
 */
function unique_get_image_attachment_count() {
 ...
 */
function unique_post_has_content( $id = 0 ) {
	$post = get_post( $id );
